...
@interface NewsAScreen ()<UITableViewDataSource,UITableViewDelegate,skip,send>
{
    NSMutableArray *loadData; //网络请求加载的数据
    
    NewsAtCell* data;  //传过来的数据
...
{
    //发起网络请求获取数据
    [CommentTool CommentToolGetMentionsWithSinceID:0 MaxID:0 Success:^(id json)
     {
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    self.sac =[tableView dequeueReusableCellWithIdentifier:name ];
    
    if (!self.sac)
    {
        
        self.sac = [[NewsAtCell alloc]initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:name];
        
        self.sac.delegate = self;
    ...
    if (loadData)
    {
        ...
        [self.sac awakeWithIndex:indexPath.section WithDic:loadData];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //获取点击的cell的内容
    NewsAtCell *ce = (NewsAtCell*)[tableView cellForRowAtIndexPath:indexPath];
